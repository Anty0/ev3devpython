...
class RobotPositionUpdater:  # TODO: implement
    def __init__(self, bricks: Bricks, bricks_controllers: BricksControllers, robot_position: dp.Position):
        ...
        if left_wheel_brick is None or right_wheel_brick is None \
            ...
            self._odometry.position = [self._position.point.x, self._position.point.y, self._position.angle.rad_z]
    ...
    def _odometry_cycle(self):
        ...
        self._position.angle.rad_z = odometry_pos[2]
    ...
    def position(self, position: dp.Position):
        ...
        if self._odometry is not None:
            self._odometry.position = [position.point.x, position.point.y, position.angle.rad_z]
