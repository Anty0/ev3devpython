...
def obstacle_avoid(shared: ShareAccessInterface, config: dict, robot_info: Robot, pilot: Pilot,
    ...
    min_distance = config['OBSTACLE_MIN_DISTANCE'] + 10 + SCANNER_DISTANCE_HEAD_POSITION.y
    ...
    if detect:
        ...
        pilot.run_percent_drive_to_distance((obstacle_width / 2) - (robot_info.size.width / 3), 0,
                                            speed_unit=target_speed)
        pilot.wait_to_stop()
        pilot.run_percent_drive_to_angle_deg(90, -course, speed_unit=target_speed)
        pilot.wait_to_stop()
...
def run_loop(shared: ShareAccessInterface):
    ...
    robot_info = shared.data.robot_info
        ...
        if (config['OBSTACLE_AVOID'] or config['COLLISION_AVOID']) and not scanner_distance.is_running:
            ...
            if scanner_distance.value(percent=True) <= config['OBSTACLE_MIN_DISTANCE']:
                ...
                if config['COLLISION_AVOID'] and \
                    ...
                    obstacle_avoid(shared, config, robot_info, pilot, scanner_distance, scanner_reflect)
